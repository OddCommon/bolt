...
export const mergeAttributes = () => {
  newPageMerge.forEach(newMergeElement => {
    ...
    if (oldMergeElement) {
      ...
      appendChild(newMergeElement.parentElement, newMergeElement);
    }
  });
};
...
const checkForBolt = (element, elementMap) => {
  if (
    getAttribute(element, selectors.BOLT_STATIC) ||
    getAttribute(element, selectors.BOLT_LINK) ||
...
const parseElement = (element, elementMap = []) => {
  return new Promise(resolve => {
    [...element.children].forEach(async (child, index) => {
      const elm = {
        parentElement: child.parentElement,
        index: index,
        element: child,
        bolt: checkForBolt(child, elementMap),
      };
      elementMap.push(elm);

      if (elm.bolt) {
        elementMap.forEach(elementItem => {
          if (elementItem.parentElement === element.parentElement) {
            elementItem.bolt = true;
          }
        });
      }

      // If more children iterate
      if (child.children.length > 0 && !elm.bolt) {
        // If the element contains bolt leave it alone
        parseElement(child, elementMap).then(data => {
          resolve(data);
        });
      } else {
        resolve(elementMap);
...
export const replace = () => {
  return new Promise(async resolve => {
    let existing = await parseElement(oldPageDOM.body, []);
    let incoming = await parseElement(newPageDOM.body, []);

    // Clear out the old
    existing.forEach((existingElement, index) => {
      if (getAttribute(existingElement.element, selectors.BOLT_ROUTER)) {
        const twin = incoming.find(newElement => {
          return (
            getAttribute(existingElement.element, selectors.BOLT_ROUTER) ===
            getAttribute(newElement.element, selectors.BOLT_ROUTER)
          );
        });

        if (!twin) {
          existingElement.element.remove();
        } else {
          const existingTwin = existing.find(e => {
            return (
              getAttribute(e.element, selectors.BOLT_ROUTER) ===
              getAttribute(twin.element, selectors.BOLT_ROUTER)
            );
          });
          if (existingTwin) {
            if (existingTwin.innerHTML !== twin.innerHTML) {
              existingTwin.innerHTML = twin.innerHTML;
            }
          }
        }
      } else {
        if (!existingElement.bolt) {
          const twin = incoming.find(newElement => {
            return existingElement.element === newElement.element;
          });
          if (!twin) {
            existingElement.element.remove();
          }
    ...
    incoming.forEach((incomingElement, index) => {
      if (getAttribute(incomingElement.element, selectors.BOLT_ROUTER)) {
        // Find the old version
        const twin = existing.find(existingElement => {
          return (
            getAttribute(existingElement.element, selectors.BOLT_ROUTER) ===
            getAttribute(incomingElement.element, selectors.BOLT_ROUTER)
          );
        });

        // Check if the old router exists if not find it's parent and append
        if (twin) {
          // If they aren't the same inners swap
          if (twin.element.innerHTML !== incomingElement.element.innerHTML) {
            twin.element.innerHTML = incomingElement.element.innerHTML;
          }
        } else {
          const parent = existing.find(e => {
            return (
              getAttribute(e.element, 'id') === getAttribute(incomingElement.parentElement, 'id')
            );
          });
          if (parent) {
            parent.element.insertBefore(
              incomingElement.element,
              parent.element.children[incomingElement.index]
            );
          } else {
            // Todo - Insert incoming elements parent and then incoming element
          }
        }
      }
      if (!incomingElement.bolt) {
        const existingParent = existing.find(parent => {
          return (
            getAttribute(parent.element, 'id') === getAttribute(incomingElement.parentElement, 'id')
          );
        });
        if (existingParent) {
          existingParent.element.insertBefore(
            incomingElement.element,
            existingParent.element.children[incomingElement.index]
          );
        }
      }
    });
