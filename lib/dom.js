...
export const parseNewPage = html => {
  ...
  newPageRouters = [...qAll(newPageDOM, `[${selectors.BOLT_ROUTER}]`)].reverse();
...
export const cacheCurrentPage = () => {
  oldPageDOM = document;
  oldPageRouters = [...qAll(oldPageDOM, `[${selectors.BOLT_ROUTER}]`)].reverse();
...
export const replace = () => {
  newPageRouters.forEach(newRouteWrapper => {
    ...
    if (oldRouterWrapper) {
      if (oldRouterWrapper.innerHTML !== newRouteWrapper.innerHTML) {
        oldRouterWrapper.innerHTML = newRouteWrapper.innerHTML;
      }
    } else {
