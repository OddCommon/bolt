...
class BoltRouter extends Eventful {
  ...
  _initialize() {
    ...
    this.#staticElements = [];
    this.#staticElements = [...document.querySelectorAll('[data-router-static]')];

    this.#staticElements = [];
    this.#staticElements = [...document.querySelectorAll('[data-router-merge]')];
  ...
  _mergeAttributes(newAttributes, oldAttributes, root) {
    // Wipe the Old elements attributes
    for (let i = 0; i < oldAttributes.length; i++) {
      root.setAttribute(oldAttributes[i].name, '');
    }

    // Merge our new attributes
    for (let i = 0; i < newAttributes.length; i++) {
      root.setAttribute(newAttributes[i].name, newAttributes[i].value);
    }
  }
  ...
  async _render(html) {
    ...
    const oldRouterWrappers = [...document.querySelectorAll('[data-router]')];
    const newRouterWrappers = [...newPage.querySelectorAll('[data-router]')];
    ...
    const oldPageStatic = [...document.querySelectorAll('[data-router-static]')];
    const oldPageMerge = [...document.querySelectorAll('[data-router-merge]')];
    ...
    const newPageStatic = [...newPage.querySelectorAll('[data-router-static]')];
    const newPageMerge = [...newPage.querySelectorAll('[data-router-merge]')];
    ...
    // Merge Static Attributes & Content
    newPageMerge.forEach(newMergeElement => {
      const oldMergeElement = oldPageMerge.find(
        oleMerge => oleMerge.dataset.routerMerge === newMergeElement.dataset.routerMerge
      );
      ...
      if (oldMergeElement) {
        ...
        this._mergeAttributes(newAttributes, oldAttributes, oldMergeElement);
      } else {
        newPage.body.appendChild(newMergeElement);
      }
    });

    // Swap Router Wrappers
    newRouterWrappers.forEach(newRouteWrapper => {
      const oldRouterWrapper = oldRouterWrappers.find(
        oldWrapper => oldWrapper.dataset.router === newRouteWrapper.dataset.router
      );
    ...
    oldRouterWrappers.forEach(oldRouterWrapper => {
      const newRouterWapper = newRouterWrappers.find(
        newWrapper => newWrapper.dataset.router === oldRouterWrapper.dataset.router
      );
    ...
    // Update Title
    document.title = newPageTitle.innerText;
