...
class BoltRouter extends EventTarget {
  ...
  constructor(parent) {
    ...
    // Private Properties
    this.#_locked = false;
    this.#_mode = strategies.AGGRESSIVE;
    this.#_cache = [];
  ...
  // Due to the nature of some bundlers we need to protect our initializing script.
  // If there is not initializing script then borderless will protect itself.
  _getFileCallerURL() {
    const err = new Error();
  ...
  _observe() {
    ...
    this.#_links.map(link => {
      if (!window.ontouchstart) {
        ['mouseover', 'touchstart'].map(event =>
          link.element.addEventListener(event, this.prefetch)
        );
      }
    ...
    if (this.#_cache[target]) {
      ...
      return new Promise(async resolve => {
        ...
        try {
          ...
          if (this.#_mode === strategies.AGGRESSIVE) {
