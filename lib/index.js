...
class BoltRouter extends Eventful {
  ...
  async _render(html) {
    ...
    if (this.#intercept) await this.#intercept;
    newPageMerge.forEach(mergeElement => {
      const oldMergeElement = oldPageMerge.find(
        oleMerge => oleMerge.dataset.routerMerge === mergeElement.dataset.routerMerge
      );

      if (oldMergeElement) {
        const newAttributes = mergeElement.attributes;
        const oldAttributes = oldMergeElement.attributes;

        // Wipe the Old elements attributes
        for (let i = 0; i < oldAttributes.length; i++) {
          oldMergeElement.setAttribute(oldAttributes[i].name, null);
        }
        ...
        newPage.body.appendChild(mergeElement);
      }
    });
          ...
          new Promise(async (resolve, reject) => {
            ...
            if (newScriptSrc) {
              if (newScriptSrc.indexOf('?') > -1) {
                newScriptSrc = `${newScriptSrc}&=${Math.round(Math.random() * 1000)}`;
              }
